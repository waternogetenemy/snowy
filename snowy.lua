...
function redraw()
  ...
  local function one_val(val, fsize)
    if type(val) == "string" and #val > 10 then val = val:sub(1, 9) .. "." end
    screen.font_size(fsize or 16)
    screen.level(15)
    screen.move(64, 40)
  ...
  screen.font_size(8)
  screen.level(3)
  screen.move(2, 7); screen.text("track " .. ti)
  ...
  if gen_mode == 0 then
    ...
    screen.font_size(16)
    screen.level(15)
    screen.move(2,  40); screen.text(division_names[params:get("t"..ti.."_div")])
    screen.move(66, 40); screen.text(params:get("t"..ti.."_swing") .. "%")
